...
public class questNew : MonoBehaviour 
{
	...
	public static questNew instance;
	...
	public void forgeGreenButton()
	{
		...
		if (PlayerParameters._instance.myPlayerParameter.stamina >= reqdStamina && hotspotsCleared.Count > 0) {
			...
			for (int i = 0; i < PlayerParameters._instance.myPlayerParameter.artefacts.Length; i++) {
				artefacts[i] = PlayerParameters._instance.myPlayerParameter.artefacts[i];
			}
...
//			Debug.Log ("Enemy Card Name = "+allEnemyCards[i]);
	...
	void SetBossStats()
	{
		BattleLogic._instance.battleType = BattleLogic.BattleType.QUEST_BOSS;
		int enemyCardNo = 0;
	...
	{
		BattleLogic._instance.battleType = BattleLogic.BattleType.QUEST;
		int enemyCardNo = 0;
		int checkingForRarityNo = 0;
	...
	public void pressedEnter()
	{
		...
		if (hotSpotId == 3) {
			...
			BattleLogic._instance.battleType = BattleLogic.BattleType.NONE;
...
//			}
	...
	{
		if(amtCleared >= myQuestManager.chapters[chapterNo - 1].area[areaNo - 1].areas[noOfTimesAreaWasCleared].clearancePointsNeeded && BattleLogic._instance.battleType == BattleLogic.BattleType.QUEST_BOSS)
		{
...
//			playerParamsDict.Add ("QuestCardFormation","");
...
//		}
	...
	IEnumerator SendAreaData(string  bossPlayed , int amountCleared ,string hotspotData, int noOfTimesHotspotCleared,  Dictionary<string, string> playerParameters , Action <bool,string> callBack )
	{
		...
		wwwForm.AddField ("tag" ,"doQuestData");
	...
	Dictionary<string, string> RewardsToSend(int hotspotId)
	{
		...
		case 8:
			//call toarm
			signalFireReward++;
//			playerParams.Add ("signal_fire",(PlayerParameters._instance.myPlayerParameter.signal_fire+1).ToString ());
			break;
		case 9:
			//orb
		...
		case 10:
			//artefact
			int itemNo = myQuestManager.chapters [chapterNo - 1].area [areaNo - 1].itemNoSetOfsetGained + PlayerParameters._instance.myPlayerParameter.avatar_no - 1;
			IncreaseItemNo (itemNo);
			string itemSet = "";
			for (int i = 0; i < PlayerParameters._instance.myPlayerParameter.artefacts.Length; i++) {
				itemSet += (artefacts [i] + ",");
			}
			itemSet = itemSet.Remove (itemSet.Length-1);
			playerParams.Add ("item_set",itemSet);
			break;
		}
		return playerParams;
	}
...
//			PlayerParameters._instance.myPlayerParameter.signal_fire++;
		...
		case 10:
			...
			int itemNo = myQuestManager.chapters[chapterNo-1].area[areaNo-1].itemNoSetOfsetGained;
			rewardImage.sprite = Artefacts._instance.gameArtifacts[itemNo-1].itemImage;
			for (int i = 0; i < PlayerParameters._instance.myPlayerParameter.artefacts.Length; i++) {
				PlayerParameters._instance.myPlayerParameter.artefacts[i] = artefacts[i];
			}
			break;
		}
	}

	void IncreaseItemNo(int itemNo)
	{
		artefacts [itemNo-1]++;
...
//						}
	...
	public void selectOrangeButton()
	{
		...
		loadingScene.Instance.questFormation();
			...
			if (isConnected)
			{
				StartCoroutine (SendAreaData( "false" , 0 ,"" , noOfTimesAreaWasCleared+1 , null , (isSuccess , msgString) => {
					if(isSuccess)
					{
						...
						QuestDataFetch._instance.LoadHotspotsPerArea("Chapter" + chapterNo  , chapterNo-1 , areaNo-1 , ((areaNo-1)*5)+noOfTimesAreaWasCleared+1 , myQuestManager);
						loadingScene.Instance.loader.SetActive (false);
						myQuestManager.chapters[chapterNo - 1].area[areaNo - 1].isCleared = false;
						myQuestManager.chapters[chapterNo - 1].area[areaNo - 1].isAllHotSpotsCleared = false;
						myQuestManager.chapters[chapterNo - 1].area[areaNo - 1].amountCleared = 0;
					...
					StartCoroutine (FetchQuestData (chapterNoClicked , (isSuccess , msgString) => {
						...
						if (isSuccess) {
							...
							if (msgString.Contains ("No data available!")) {
								...
								QuestDataFetch._instance.Load ("Chapter" + chapterNoClicked , chapterNoClicked - 1, areaLines, myQuestManager);
...
//				myQuestManager.chapters [chapterNo - 1].area [i].isCleared = true;
	...
	{
		WWWForm wwwForm = new WWWForm ();
		wwwForm.AddField ("tag" ,"doGetAllQuestData");
